...
let posterPaths = "https://image.tmdb.org/t/p/w370_and_h556_bestv2";
let backgroundPaths = "http://image.tmdb.org/t/p/w1280";
let url = "https://api.themoviedb.org/3/discover/movie?";
let key = "&api_key=c418b1309024eb7dabe7c01494b70511";
let urlTV = "https://api.themoviedb.org/3/discover/tv?";
let moreTVinfo =
  "https://api.themoviedb.org/3/tv/  +tvshow id+  ?&api_key=c418b1309024eb7dabe7c01494b70511";
let movieCast = "https://api.themoviedb.org/3/movie/";
let actorInfo = "https://api.themoviedb.org/3/discover/movie?&with_cast=";
let imdbLink = "http://www.imdb.com/title/";
let date = new Date();
...
function search(search) {
  ...
  $(".tv").remove();
  let searchurl =
    "https://api.themoviedb.org/3/search/multi?api_key=c418b1309024eb7dabe7c01494b70511&query=";
  $.getJSON(searchurl + search, function (data) {
    for (let i = 0; i < data.results.length; i++) {
      let id = data.results[i].id;
      let title = data.results[i].title;
      let rating = data.results[i].vote_average;
      let poster = posterPaths + data.results[i].poster_path;
...
function movieInfo(id) {
  $.getJSON(movieCast + id + "/casts?" + key, function (json) {
    ...
    let infoURL =
      ...
      id +
      "?&api_key=c418b1309024eb7dabe7c01494b70511";
    $.getJSON(infoURL, function (data) {
      ...
      let imdb = imdbLink + data.imdb_id;
...
function tvInfo(id) {
  ...
  $.getJSON(infoURL, function (data) {
    ...
    //let selected = $( ".season option:selected" ).value();
...
function seriesInfo(id, num) {
  ...
  $.getJSON(seriesURL, function (data) {
    for (let i = 0; i < data.episodes.length; i++) {
      let seasonname = data.name;
      let seasonoverview = data.overview;
      let episode = data.episodes[i].name;
      let overview = data.episodes[i].overview;
      let airdate = data.episodes[i].air_date;
